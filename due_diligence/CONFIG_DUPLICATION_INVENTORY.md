...
| 2 | ~~ERM client config path default in core runtime~~ | RESOLVED — `--master-config` no longer defaults to a client; `OpsEngine` resolves the run's own client configuration through `EffectiveConfigResolver.client_config_for()` | Runtime context selection + platform default profile | — | Done | Closed |
...
| 7 | ~~Client legal/reporting constants duplicated~~ | RESOLVED for production — both documents retired to `tests/fixtures/legacy_client/`; a client's constants come from its own onboarding answers, injected once via the entity that holds the originator role | Client config (single source) + regime injection | — | Done | Closed |
| 8 | Conflicting static/reporting dates | Neither document is production configuration any more, so nothing conflicts at runtime. The underlying gap stands: `IVSS2` (investor-report period end) has no governed owner, and must come from run context rather than standing client configuration when a live Annex 12 route exists | Run context (never client standing config) | Medium | Pass 2 | Near-term |
