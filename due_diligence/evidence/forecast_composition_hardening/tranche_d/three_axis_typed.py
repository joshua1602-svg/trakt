...
#: This file lives one level below the pack, so the controls it reads are in the
#: parent. It is a COPY of ../three_axis.py extended with a fourth axis, and it
#: is a separate file for a reason worth recording: ../three_axis.py is hashed in
#: the V1 MANIFEST, and editing a hashed artefact in place is how the manifest
#: verification silently began failing during Tranche D. The instrument that
#: produced the Tranche D typed measurements is THIS file; the hashed one is
#: unchanged and still verifies.
HERE = Path(__file__).resolve().parent
PACK = HERE.parent
EXPECTATIONS = PACK / "frozen_expectations.yaml"
...
ANSWER_TYPES = PACK / "answer_types_44.yaml"
