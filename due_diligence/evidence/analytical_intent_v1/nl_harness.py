"""NL robustness harness — runs the bank through the production entrypoint.

    python3 nl_harness.py <book> <arm> <out.json>

    book : alderbridge | kestrelmoor
    arm  : production   — exactly as deployed (zero-cost-first ON, LLM enabled)
           forced_llm   — the ONE documented cost control disabled in-process so
                          the LLM parses every question. Not production; run so
                          §4's repeat-variance has something to vary on questions
                          the deterministic parser resolves for free.

No production file is modified. The forced arm wraps ParsedQuestion.parse inside
this process only.
"""
...
# The LLM parser ON — this is the whole point of the exercise.
os.environ["MI_AGENT_LLM_PARSER"] = "on"
os.environ["MI_AGENT_LLM_ENABLED"] = "1"
