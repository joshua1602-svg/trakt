...
RESULT = HERE / "change_form_result.json"
...
def adjudicate(case: Mapping[str, Any], payload: Optional[Mapping[str, Any]],
    ...
    if not row["temporal_ok"]:
        ...
        row["problems"].append(
            f"temporal: expected one of {case['temporal_forms']}, got {form!r} "
            f"- the two-state relationship the question states is not preserved")
    ...
    if want and read["lens"] != want and not read["blocking_ambiguities"]:
        silent.append("scope")
    if not row["temporal_ok"] and not read["blocking_ambiguities"]:
        silent.append("temporal_pair")
...
STAND_INS = {
    "CF01": _standin("material_summary", operation="summary"),
    "CF05": _standin("material_summary", operation="summary", lens="direct"),
    "CF06": _standin("metric_delta", measures=["balance"]),
    "CF08": _standin("metric_delta", measures=["loan"]),
    "CF11": _standin("attribution", measures=["balance"], operation="bridge",
                     capability="funded_bridge"),
    "CF18": _standin("level_comparison", measures=["outstanding_balance"],
                     lens="acquired", operation="compare",
