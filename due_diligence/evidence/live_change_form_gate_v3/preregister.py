#!/usr/bin/env python3
"""Pre-register live_change_form_gate_v3. Run ONCE, before Opus.

WHY v3 AND NOT v2. v2 was pre-registered and hashed BEFORE the current-anchor
temporal contract was closed, and its stated reason for not gating temporal was
that "the deterministic owners do not currently honour a `current` anchor for
material_summary or attribution". That sentence is now FALSE for
`material_summary`: its runtime completes a `current` anchor through the
resolver's own `current_vs_previous`. An already-hashed bank is never edited after
a semantic policy change — doing so would let the expectations follow the result —
so v2 is retired unrun and this is a fresh pre-registration. The EIGHTEEN QUESTIONS
ARE UNCHANGED from v2; only the temporal expectation is corrected.
...
MANIFEST = HERE / "change_form_v3_bank_manifest.json"
HASH = HERE / "change_form_v3_bank_manifest.sha256"
...
#: THE TEMPORAL EXPECTATION, PER FORM — and gated in v3, unlike v2.
#:
#: The reader supplies the anchor or an explicit pair; the analytical owner
#: supplies any comparison state the FORM itself intrinsically requires. That rule
#: divides the four forms in two, and the division is a fact about the owners
#: rather than a preference:
#:
#:   material_summary  a comparison state is part of what the form IS, and its
#:                     runtime completes a `current` anchor via the resolver's
#:                     `current_vs_previous`. So `current` is VALID, and requiring
#:                     the interpreter to author the other end would be asking it
#:                     to duplicate a deterministic responsibility.
#:   attribution       `bridge` owns its own window, so one anchor already denotes
#:                     two states and `current` is a VALID reading. Execution
#:                     connectivity for a governed funded_bridge plan does not
#:                     exist yet; that is a recorded connectivity item and not an
#:                     interpretation error, so it does not change what is valid
#:                     to READ.
#:   metric_delta      names one quantity and asks how much it moved. Its owner
#:                     requires a period richer than a single point, and a bare
#:                     `current` is still refused — so `current` is NOT valid.
#:   level_comparison  asks for two states the reader has in mind. Its owner wants
#:                     the pair named, so `current` is NOT valid.
ANCHOR_OR_PAIR = ["current", "relative_pair", "previous_reporting_period",
                  "explicit_period"]
PAIR_ONLY = ["relative_pair", "previous_reporting_period", "explicit_period"]

TEMPORAL_BY_FORM = {
    "material_summary": ANCHOR_OR_PAIR,
    "attribution": ANCHOR_OR_PAIR,
    "metric_delta": PAIR_ONLY,
    "level_comparison": PAIR_ONLY,
}
...
def build() -> dict:
    for case in CASES:
        case["filters"] = []
        case["dimensions"] = []
        case["temporal_forms"] = list(TEMPORAL_BY_FORM[case["change_form"]])
    return {
        "bank_id": BANK_ID,
        "model": MODEL,
        "baseline_sha": BASELINE_SHA,
        "supersedes": "live_change_form_gate_v1 (immutable, 17/18 change_form); retires live_change_form_gate_v2 UNRUN, whose temporal pre-registration was invalidated by the current-anchor contract being closed",
        "what_this_is":
        ...
        "temporal_is_gated_per_form": {
            "material_summary": ANCHOR_OR_PAIR,
            "attribution": ANCHOR_OR_PAIR,
            "metric_delta": PAIR_ONLY,
            "level_comparison": PAIR_ONLY,
            "why":
                "A `current` anchor is VALID for the two forms whose owner "
                "supplies the comparison state (material_summary completes it "
                "via current_vs_previous; attribution's bridge window is "
                "capability-owned) and INVALID for the two whose owner requires "
                "the pair to be named. v2 could not gate this because the "
                "contract was unsettled; it is settled now.",
        },
        "attribution_connectivity_note":
            "A governed-plan runtime for funded_bridge does not exist. The "
            "calculation owner already accepts current_vs_previous — it shares "
            "`resolve_periods` with every other period-change figure — so this "
            "is plan connectivity, not missing temporal behaviour. It is a "
            "bounded item for the serving sprint and does not affect what is "
            "valid to interpret.",
