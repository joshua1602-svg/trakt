...
RESULT = HERE / "change_form_v3_result.json"
...
def adjudicate(case: Mapping[str, Any], payload: Optional[Mapping[str, Any]],
    ...
    # An accepted `current` is an ANCHOR the owner completes, not a pair the
    # reader stated. Reporting both as "PAIR_PRESERVED" would hide which of the
    # two actually happened, which is the distinction this contract rests on.
    if row["temporal_ok"]:
        row["period_result"] = ("ANCHOR_ACCEPTED" if form == "current"
                                else "PAIR_PRESERVED")
    elif form in _SINGLE_STATE:
        row["period_result"] = "SINGLE_STATE_REJECTED"
    else:
        row["period_result"] = f"UNEXPECTED:{form}"
    # GATED AGAIN IN V3, and PER FORM. v2 could not gate temporal because the
    # deterministic contract for a `current` anchor was unsettled. It is settled
    # now: a material summary completes a `current` anchor through the resolver's
    # own current_vs_previous, and an attribution's window is capability-owned —
    # so for those two forms `current` IS a valid reading and the manifest lists
    # it. A metric delta and a level comparison still need the pair their owners
    # require, so for them `current` is a genuine temporal miss. The acceptable
    # set is therefore read from the case, not assumed to be the same for all.
    if not row["temporal_ok"]:
        row["failure_classes"].append("C")
        row["problems"].append(
            f"temporal: {form!r} is not one of {case['temporal_forms']} for "
            f"change_form {case['change_form']!r}")
