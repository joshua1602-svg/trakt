...
RESULT = HERE / "change_form_v4_result.json"
...
def adjudicate(case: Mapping[str, Any], payload: Optional[Mapping[str, Any]],
    ...
    # -- C. the temporal ROUTE, scored from key presence ------------------ #
    # `time_form` is None exactly when the payload carried no `form` key, which is
    # the ABSENT route. v3 scored that as an unexpected value; it is a legitimate
    # reading wherever the analytical form owns the comparison window, so v4 scores
    # it as its own route against the per-form accept list.
    form = read["time_form"]
    absent = form is None
    route = ("OWNER_DEFAULT_ABSENT" if absent
             else "EXPLICIT_ANCHOR" if form == "current"
             else "EXPLICIT_PAIR" if form in ("relative_pair",
                                              "previous_reporting_period",
                                              "explicit_period")
             else f"OTHER:{form}")
    accept = case["temporal_accept"]
    row["temporal_ok"] = ("ABSENT" in accept) if absent else (form in accept)
    row["interpreted_time_present"] = not absent
    row["temporal_route"] = route
    row["temporal_intent"] = case["temporal_intent"]
    row["period_result"] = route if row["temporal_ok"] else "TEMPORAL_FAIL"
    if not row["temporal_ok"]:
        row["failure_classes"].append("C")
        row["problems"].append(
            f"temporal: route {route} is not valid for change_form "
            f"{case['change_form']!r}, which accepts {accept}")
    ...
    # A FILTER THE WORDING SUPPORTS IS THE READER'S, NOT AN INVENTION. v3 pinned
    # an empty set on every case and then counted a governed predicate the question
    # actually asked for ("live funded loans") as invented. The permitted set is
    # per case, and resolved through the vocabulary so an alias is not a miss.
    permitted = {_canonical(c) or c for c in (case.get("filters_permitted") or ())}
    invented = [f for f in read["filters"]
                if (_canonical(f.get("concept")) or f.get("concept"))
                not in permitted]
    row["filters_permitted"] = sorted(permitted)
    row["invented_filters"] = invented
    row["reader_filters"] = [f for f in read["filters"] if f not in invented]
    row["invented_dimensions"] = read["dimensions"]
    if invented:
        row["failure_classes"].append("E")
        row["problems"].append(
            f"invented filter(s) the wording does not support: {invented}")
    if read["dimensions"]:
        row["failure_classes"].append("E")
        row["problems"].append(
...
def _standin(change_form: str, *, measures=(), lens="all",
    ...
    """`form=None` omits the `time` block entirely — the ABSENT route."""
    body = {
    ...
    if form is not None:
        body["time"] = {"form": form, "periods_back": 1}
    return body


STAND_INS = {
    # one per temporal route, so the dry run proves the discrimination
    "W01": _standin("material_summary", operation="summary"),            # pair
    "W02": _standin("material_summary", operation="summary", form="current"),
    "W03": _standin("material_summary", operation="summary", form=None),  # absent
    "W05": _standin("metric_delta", measures=["balance"]),
    "W10": _standin("attribution", measures=["balance"], operation="bridge",
                    capability="funded_bridge"),
    "W12": _standin("attribution", measures=["bridge_component"],
                    operation="bridge", capability="funded_bridge", form=None),
    "W14": _standin("level_comparison", measures=["outstanding_balance"],
                    operation="compare", capability="generic_analysis"),
}
...
def _totals(rows: List[Mapping[str, Any]],
    ...
    return {
        ...
        "temporal_route_valid":
            f"{sum(1 for r in rows if r.get('temporal_ok'))} / {len(rows)}",
        "temporal_routes_taken": {
            route: sum(1 for r in rows if r.get("temporal_route") == route)
            for route in sorted({r.get("temporal_route") for r in rows
                                 if r.get("temporal_route")})},
        "interpreted_time_present":
            f"{sum(1 for r in rows if r.get('interpreted_time_present'))} / {len(rows)}",
