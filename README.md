...
python engine/orchestrator/trakt_run.py \
  --mode annex12 \
  --input loan_portfolio_112025.csv \
  --config <this client's investor-report configuration>
...
| File | Role |
|------|------|
| _(generated per client)_ | Master client config — identity, transformations, enrichment rules, UI branding. Created by Client Onboarding's `activate()`, versioned and hashed per client in the operations store; there is no repository copy and no default |
| _(generated per client)_ | ESMA Annex 12 deal metadata and structural overlay, generated alongside it |
...
trakt/
  ...
  config/
    ...
    client/                          # Examples and registries only — a live
                                       # client's configuration is generated by
                                       # Client Onboarding into the operations
                                       # store, never kept here
