...
DEFAULT_CLIENT_CONFIG = Path("config/client/config_client_ERE.yaml")
...
def materialise_effective_config(*, base_config_path: Path,
    ...
    for key, value in (overrides or {}).items():
        ...
        if not deep_set(cfg, key, value):
            cfg[key] = value
    if constants:
        merged_c = dict((cfg.setdefault("defaults", {})
                         ).setdefault("regulatory_constants", {}) or {})
        merged_c.update(constants)
        cfg["defaults"]["regulatory_constants"] = merged_c
    if nd_defaults:
        # A no-data code is applied by the projector while it builds the return
        # and is never written back to the canonical, which is exactly the
        # boundary these decisions are about: the lender still does not supply
        # the field, and the regulator is still told why.
        merged = dict((cfg.setdefault("defaults", {})
                       ).setdefault("nd_defaults", {}) or {})
        merged.update(nd_defaults)
        cfg["defaults"]["nd_defaults"] = merged
        # An operator approval IS the stated rationale the projector requires
        # before a client-level code may override the asset pack's.
        rationale = dict(cfg.get("nd_override_rationale") or {})
        for field, code in nd_defaults.items():
            rationale.setdefault(
                field, f"approved in the Operations Control Centre ({code})")
        cfg["nd_override_rationale"] = rationale
