 ...
 *
 * This is the same decision form the standalone review screen used, lifted out
 * so it can live inside the delivery's own case file: an operator answers a
 * question about a delivery while looking at the delivery, not on a page of its
 * own with no context.
 */
export function DecisionPanel({
  ...
  const { data: review, error, loading, reload } = useLoad(
    () => client.getReview(decisionId),
    [decisionId],
  );
  ...
  const isMapping =
    (review?.kind.includes("mapping") ?? false) || review?.kind === "client_rule";
  ...
  function handleConfirm() {
    ...
    void submit(choice === OTHER ? "amend" : "approve", valueToSend, "");
  }

  if (loading && !review) return <Loading />;
  if (error && !review) return <ErrorNote message={error} onRetry={() => void reload()} />;
  if (!review) return null;

  return (
    <div className="space-y-5 rounded-2xl border border-amber-200 bg-amber-50/40 p-5">
      <h4 className="text-base font-semibold leading-relaxed text-stone-900">{review.question}</h4>
          ...
          <div className="space-y-2">
            {review.options.map((option) => (
              ...
              >
                <input
                  type="radio"
                  name={`option-${review.decision_id}`}
                  value={option.value}
          ...
          {rejectOpen && (
            <div className="rounded-xl border border-stone-200 bg-white p-4">
              <label
                ...
                htmlFor={`reject-${review.decision_id}`}
              >
                {copy.reviews.rejectReason}
              </label>
              <textarea
                id={`reject-${review.decision_id}`}
                rows={3}
                value={rejectReason}
