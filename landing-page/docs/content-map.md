...
| Card | Class | Evidence |
|---|---|---|
| **Microsoft 365 Copilot and Teams** — "Access Trakt's declarative agent through supported Microsoft 365 Copilot and Teams surfaces — ask portfolio questions, request reports and retrieve current management information without leaving the tools you already use." | Evidenced | One card, not two, because there is one artefact: a **declarative agent**, packaged as a Teams app (`deploy/copilot-agent/manifest.json`) and surfaced through Microsoft 365 Copilot, exposing exactly three actions — `askTraktMi`, `getLatestInvestorDeck`, `getLatestCanonicalTape` (`docs/copilot_v1_implementation.md`). **Revised during hardening:** the previous separate "Teams" card implied a standalone native Teams bot. No such bot exists — no bot registration, no adaptive cards, no proactive messaging — so the copy now names the *surfaces* the declarative agent reaches. |
...
| **Omnichannel Intelligence** — "Provide governed portfolio answers through the Microsoft 365 Copilot and Teams surfaces, the Trakt workspace, APIs and scheduled reporting." | Evidenced | As per section F, plus `mi_agent_api/auth.py` and `copilot_auth.py` for role-based delivery. Wording aligned with the channel card above — "Copilot, Teams" became "the Microsoft 365 Copilot and Teams surfaces" for the same reason. |
...
| "We use your details only to respond, and do not add you to a marketing list." | Deployment | **Operational commitment.** True of the delivery adapters as implemented: exactly one destination, no list subscription, and the app itself retains nothing (no lead is written to disk in production; the idempotency record holds a hash for ten minutes). Honouring it beyond that is a business undertaking — if lead handling changes, this line must change with it. See README § "Who owns incoming leads". |
