...
def section_1():
    ...
    rows = [
        ...
        ["Gate 4b — Delivery Preflight",
         ...
         f"{enum_issues} enum code-mapping errors (property_type, interest_rate_type, purpose)",
         badge("FAIL", "fail")],
        ["Gate 5 — XML Builder",
         f"{XML_PATH.name}  ({xml_size_kb:.1f} KB)",
         f"{xml_records} UndrlygXpsrRcrd elements; XSD validation failed (enum values)",
         badge("PARTIAL", "warn")],
    ]
...
def section_6():
    ...
    <div class='callout warn'>
      ...
      The enum_mapping.yaml was not updated for this portfolio, causing 108 blocking
      enum errors. XML was still generated by Gate 5 with the raw values.
    </div>"""
