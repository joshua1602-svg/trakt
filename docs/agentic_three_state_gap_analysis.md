...
**Revision 2 (operator clarifications incorporated):** (1) the gate scripts (Gate 1 mapping kernel, Gate 2/3 primitives, Gate 3's 48-rule library, Gate 4 projector, Gate 5 XSD-validating builders) are classified as **valid frozen deterministic components**, not deprecated code — the legacy surface is the **Streamlit family**; (2) the Annex 2 XSD position has been re-verified by executing the annex2 test suite — full-document DRAFT-XSD validation **passes with dummy values** (see §3); (3) the MI API data-source fall-through mechanism is spelled out precisely (see §9).
...
| `engine/gate_2_transform`, `gate_3_validation` primitives | **Shared frozen deterministic components** — valid, cherry-picked via adapters | `transformation_agent/gate2_adapter.py:35`, `validation_agent/rules_adapter.py:31` |
| `engine/gate_3_validation/validate_business_rules.py` (48-rule library) | **Valid rule library, not yet absorbed by the agentic validation agent** | `:204-720` vs `rules_adapter.py:372-437` |
| `engine/gate_4_projection/regime_projector.py` | **Valid frozen component, on the active path** — the debt is the integration form (subprocess, inconsistent flags, hard-coded client config), not the script's validity | `engine/orchestrator_agent/adapters.py:139-153`, `engine/assembler_agent.py:44` |
...
| `engine/gate_4b_delivery`, `gate_5_delivery` | **Valid frozen components, dormant** — Gate 5 retains the only production-grade full-document XSD validation call in a shipping builder | `gate_5_delivery/xml_builder_annex2.py:682-683` |
...
| `mi_agent/streamlit_mi_agent.py` | **Legacy (Streamlit)** — thin superseded UI over the same workflow | `docs/phase6e...md:242` |
| `analytics/` (Streamlit ERM dashboard) | **Legacy (Streamlit), still actively deployed** with the repo's best CI | `.github/workflows/deploy_streamlit_dashboard.yml` |
| `ui/onboarding_review.py` (Streamlit onboarding workbench) | **Legacy (Streamlit)** — but currently the primary human-review surface for onboarding; retiring it requires the review workflow to move (operator console / React) | `ui/onboarding_review.py:1-22`; CLI fallback `cli/onboarding_review_cli.py` |
...
**Validation [Confirmed].** Validation agent produces the `40_*–46_*` package (results, readiness, issues, lineage, blocker diagnostics — `validation_agent.py:587-690`). **Coverage caveat:** the agentic business-rule set is 7 hand-coded checks vs the 48 declarative rules in the frozen (and still authoritative) Gate 3 library (`rules_adapter.py:372-437` vs `gate_3_validation/validate_business_rules.py:204-720`) — absorption work outstanding [Confirmed].
...
**ESMA Annex 2 XML generation [Confirmed — structurally proven, deliberately gated].** Production produces regime *CSV* via the Gate 4 projector subprocess; the delivery/XML agent is a readiness stage that hard-codes `xml_generated: false` / `ready_for_xml_delivery: false` (`delivery_xml_agent.py:787-804, 856-857`), and all four preview modes are disabled by default in `config/delivery/xml_preview_policy.yaml`.

The XSD position — **re-verified for this review by executing the annex2 test suite** (`tests/test_xsd_structured_synthetic.py` + `tests/test_annex2_path_acceptance_gate.py`: 30/30 pass) — is materially stronger than the path map's headline `confirmed: 11` counter suggests, because that counter tracks only one of three separate axes:

1. **Path readiness:** 100 of 107 field paths are XSD-validated and formally accepted for the new builder — 11 `sample_confirmed` (verified against the vendored ESMA sample XML) + 89 `accepted_for_builder` (workbook-derived, each independently re-validated against the XSD by `test_annex2_path_acceptance_gate.py:test_accepted_paths_revalidate_against_xsd`). The remaining 7: 5 `needs_manual_review`, 2 `rejected` (RREC1/RREC2 multi-code pollution). `builder_eligible_behind_structure_gate: 100` (`config/delivery/annex2_field_xsd_path_map.yaml`).
2. **Full-document structural validation:** the synthetic schema-test mode builds the complete mandatory ESMA tree with type-valid dummy values and **passes full DRAFT-XSD document validation with zero errors** (verified by generating the `116_xsd_structured_synthetic_xsd_validation.json` report during this review: `xsd_validation_passed: true`, `error_count: 0`, 102 fields populated). The nested `Document/…/UndrlygXpsrRcrd/…/Coll` structure, ND wrappers, and RREC-under-Coll hierarchy are proven.
3. **Production readiness:** `production_ready: 0` for all 107 fields is a **deliberate governance stance**, not a structural failure — the path map explicitly separates path readiness from data readiness (`promotion_summary.note`), and production gates stay false until real-data economic values, formal per-field acceptance, and the final (non-DRAFT — `DRAFT1auth.099.001.04`) ESMA schema are in place.

So the remaining Annex 2 work is **promotion and real-data readiness, not XML capability**: resolve the 7 outstanding fields, run the accepted structure against real canonical data, complete the formal acceptance, and lift the delivery agent from readiness stage to producer. Docs and code agree throughout (one stale count: `annex2_production_xml_structure_contract.md:41` says 8 sample-confirmed vs actual 11).
...
| Generate/retrieve ESMA Annex 2 XML | Production XML deliberately gated; structure proven (100/107 paths accepted; full synthetic mandatory-tree document passes DRAFT XSD with 0 errors — re-verified by running the annex2 tests) | `delivery_xml_agent.py:787-804`; `annex2_field_xsd_path_map.yaml` (`builder_eligible_behind_structure_gate: 100`); `tests/test_xsd_structured_synthetic.py`, `tests/test_annex2_path_acceptance_gate.py` | Remaining work is promotion, not capability: 7 outstanding fields, real-data readiness, formal acceptance, final (non-DRAFT) schema, producer wiring + exposure endpoint | Medium (product promotion) | Complete acceptance of outstanding fields; run accepted structure on real canonical data; lift delivery agent to producer; then expose behind approval-aware retrieval | XML v2 promotion, artefact registry | Medium |
...
| ESMA Annex 2 XML | ✖ production (deliberately gated: `xml_generated: false`); ✔ structurally proven — synthetic full-tree document passes DRAFT XSD, 0 errors | ✔ | ◐ | ✖ | ◐ (elaborate readiness gates exist — the best-governed artefact, and the only ungenerated one) | n/a | ✖ | ✖ | ✖ | Promotion pending, not capability: 7/107 paths outstanding (5 manual review, 2 rejected), real-data readiness, final non-DRAFT schema; three coexisting builders to consolidate |
...
- **Fail-safe defaults:** mostly good (fail-closed routing, fail-closed auth toggle), with one bad exception — the MI API's data-source resolution falls through silently to demo data [Confirmed, mechanism re-verified]. The chain in `mi_agent_api/data_source.py` (`resolve_data_source`, `:169-183`) is: `MI_AGENT_ANALYTICS_DATASET` → platform canonical (`MI_AGENT_PLATFORM_URI` blob / `MI_AGENT_PLATFORM_CANONICAL` / `MI_AGENT_PLATFORM_DIR` / conventional `out_platform/`) → central tape (`MI_AGENT_CENTRAL_TAPE` / onboarding-root globs) → `MI_AGENT_DATA_CSV` → **bundled `synthetic_demo/**/*canonical_typed.csv`** (`_find_demo_csv`, `:158-164`). Every step returns `None` on failure rather than raising: a wrong or unreachable `MI_AGENT_PLATFORM_URI` (`storage.exists()` false → `None`, `:69-70`), a storage-module import failure (`:63-66`), or app settings dropped in a redeploy all degrade the production API to serving the synthetic demo portfolio with HTTP 200s. The only signal is the `dataSource` kind on `/health` (`KIND_SYNTHETIC_DEMO`, `:53`). Target state (operator-confirmed intent): the API must **fail closed on anything but live data** — demo fallback only behind an explicit opt-in flag, never as the default resolution tail.

---

## 10. Legacy-contamination assessment

**Classification note (operator-confirmed):** the gate scripts themselves — Gate 1 deterministic mapping, Gate 2/3 transformation and validation primitives, the Gate 3 48-rule library, the Gate 4 projector, and the Gate 5 XSD-validating XML builders — are **valid frozen deterministic components**, not deprecated code. The genuine legacy surface is the **Streamlit family**: `analytics/` (ERM dashboard), `mi_agent/streamlit_mi_agent.py`, `ui/onboarding_review.py` (onboarding workbench), and their deployment artefacts (`Dockerfile.streamlit`, `deploy-streamlit.sh`, `deploy_streamlit_dashboard.yml`). The findings below are therefore about *integration form and duplication*, not about the validity of the gate logic.

**Active dependencies on frozen gate components [Confirmed]:**
1. **The agentic pipeline's projection step is the Gate 4 projector, executed as a subprocess** (`orchestrator_agent/adapters.py:139-153` → `assembler_agent.py:44,102-116`; again from `regime_runner.py:32-38`). The script is valid; the integration is the debt: no subprocess timeout, `allow_unreviewed=True` from the orchestrator vs `False` from the router (inconsistent enum-review policy), the hard-coded ERM-UK client config for all tenants, and a transitive reach into the LLM enum agent (`regime_projector.py:43,48`) that the agentic `projection_agent/gate4_adapter.py:20-23` deliberately avoids.
2. **Gate 1 `semantic_alignment.py` is a shared kernel** — 17 agentic import sites. Treating it as removable legacy would break the entire onboarding path; it should be formally recognised (and eventually relocated) as the deterministic mapping kernel.
3. Gate 2/3 primitives are consumed via deliberate, documented adapters that refuse the dangerous parts (ND5 silent fill, `sys.exit(2)` hard gates) — a healthy reuse pattern for frozen components.
4. **The Gate 3 48-rule library remains the authoritative business-rule set.** The agentic validation agent has so far absorbed only 7 hand-coded checks (`rules_adapter.py:372-437`) — the gap is *absorption/parity work still to do*, not a reason to bypass the frozen library.
...
**Migration risks:**
- The legacy Streamlit dashboard remains deployed on every `main` push with the repo's best CI while being documented as "do not migrate yet" legacy — a standing risk that legacy behaviour continues to be what some clients see. Note also that the Streamlit onboarding workbench (`ui/onboarding_review.py`) is legacy *surface* but currently the primary human-review tool for onboarding — its retirement needs a replacement workflow (operator console or React) planned first.
- The validation-coverage gap (48 authoritative rules in the frozen Gate 3 library vs 7 absorbed by the validation agent) means any consumer relying solely on the agentic validation package silently loses rule coverage; nothing versions or reconciles the two rule sets.
...
| Fail-fast on data-source misconfiguration | Synthetic demo is the silent resolution tail; blob-miss/env-loss degrade to demo data with 200s (only signal: `/health` dataSource kind) | all | Demo data presented as client data via Copilot | Fail closed on anything but live data; demo behind explicit opt-in flag only | `data_source.py:158-183` |
...
**Excluded from the first pilot:** deck/tape *generation* (needs the job model); ESMA XML anything (production promotion deliberately incomplete — structure is proven with synthetic values, but real-data readiness, formal acceptance, and the final non-DRAFT schema are outstanding); canonical tape export (bulk client data — needs the secure-retrieval controls); all operator/approval actions; scenario what-ifs (defer until answers can be clearly labelled hypothetical); anything touching pipeline/forecast datasets until the cross-client deck-source fallback (`mi_agent_pptx/cli.py:222`) class of issue is ruled out for the query path.
